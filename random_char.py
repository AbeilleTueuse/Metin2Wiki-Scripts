...
def create_char(n=0):
    ...
    weapons = {
        vnum: data[1]
        for vnum, data in WEAPON_DATA.items()
        if data[0] in ALLOWED_WEAPONS[character_race]
    }
...
def create_fights(number_of_fights=1000):
    ...
    for _ in range(number_of_fights):
        if rd.randint(0, 1):
            ...
            if attacker[STATE] == POLYMORPH:
                ...
                skills_to_use = ALLOWED_SKILLS[attacker_class].copy()
                ...
                skills_to_use = [
                    ...
                    if attacker[SKILL_MAPPING[skill]]
                ]
    ...
    return characters, fights


def create_file(n):
    characters, fights = create_fights(n)

    characters = [
        f"{character[NAME]}: " + json.dumps(character).replace(" ", "")
        for character in characters
    ]

    content = "const characters = {\n\t" + ",\n\t".join(characters) + "\n}\n"

    fights = [
        f'{{attackerName: "{fight[0]}", victimName: "{fight[1]}", attackType: "{fight[2]}", resultsName: "fight{index}"}}'
